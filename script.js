...
function RoundingSuggestion(tipRate, hoursWorked, totalHours) {
  ...
  while (sumRoundedTips !== totalTips && iterations < 50) {
    ...
    for (i = 0; i < hoursWorked.length; i++) {
      ...
      sumRoundedTips +=
        Math.floor(numHoursWorked * tipRate) +
        Math.floor(
          (numHoursWorked * tipRate - Math.floor(numHoursWorked * tipRate)) /
            roundingFloor
        );
    }
...
function updateTable(e) {
  ...
  var myCurrentRate = 0;
  if (
    !isNaN(Number(myInputTips.value)) &&
    Number(myInputTips.value) > 0 &&
    runningTotal > 0
  ) {
  ...
  if (myCurrentRate > 0) {
    ...
    var roundingFloor = RoundingSuggestion(
      myCurrentRate,
      myHours,
      runningTotal
    );
    for (i = 0; i < myHours.length; i++) {
      ...
      if (!isNaN(Number(myHours[i].value)) && Number(myHours[i].value) > 0) {
        var optimalRoundedTip =
          Math.floor(Number(myHours[i].value) * myCurrentRate) +
          Math.floor(
            (Number(myHours[i].value) * myCurrentRate -
              Math.floor(Number(myHours[i].value) * myCurrentRate)) /
              roundingFloor
          );
...
function infoToggle() {
  if (infoScreen) {
    ...
    if (myTipRateTable.hidden === true) {
      var dimmerHeight =
        1 +
        myTotalTipsInputTable.getBoundingClientRect().bottom -
        mainTable.getBoundingClientRect().top;
    } else {
      var dimmerHeight =
        1 +
        myTipRateTable.getBoundingClientRect().bottom -
        mainTable.getBoundingClientRect().top;
    }
    myDimmer.setAttribute(
      "style",
      "position: relative; left: -50%;height:" + dimmerHeight + "px;"
    );
    myDimmer.hidden = false;
    // Display info bubbles
    // Speech bubble 1:
    mySpeechBubble[0].setAttribute(
      "class",
      "speech-bubble unselectable animate__animated animate__bounceIn"
    );
    mySpeechBubble[0].hidden = false;
    // Speech bubble 2:
    mySpeechBubble[1].setAttribute(
      "class",
      "speech-bubble unselectable animate__animated animate__bounceIn animate__delay-1s"
    );
    mySpeechBubble[1].hidden = false;
    // Speech bubble 3:
    mySpeechBubble[2].setAttribute(
      "class",
      "speech-bubble unselectable animate__animated animate__bounceIn animate__delay-2s"
    );
    mySpeechBubble[2].hidden = false;
    // Speech bubble 4:
    myCreditsTimer = window.setTimeout(function () {
      mySpeechBubble[3].setAttribute(
        "class",
        "speech-bubble unselectable animate__animated animate__fadeInLeft"
      );
...
function removeHelpBubble() {
  mySpeechBubble[4].setAttribute(
    "class",
    "speech-bubble unselectable animate__animated animate__fadeOut animate__faster"
  );
...
mySpeechBubble[4].setAttribute(
  "class",
  "speech-bubble unselectable animate__animated animate__fadeIn animate__delay-2s"
);
